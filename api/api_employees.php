...
switch ($action) {
    ...
    case 'add_edit':
        ...
        $user_id = isset($_POST['user_id']) ? (int) $_POST['user_id'] : 0; // The associated user account
        ...
        if ($employee_id === 0) { // Add new employee
            $sql = "INSERT INTO employees (user_id, first_name, last_name, department_id, designation_id, date_of_joining, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $params = [$user_id, $first_name, $last_name, $department_id, $designation_id, $date_of_joining, $status];
        } else { // Edit existing employee
            $sql = "UPDATE employees SET user_id = ?, first_name = ?, last_name = ?, department_id = ?, designation_id = ?, date_of_joining = ?, status = ? WHERE id = ? AND user_id IN (SELECT id FROM users WHERE company_id = ?)";
            $params = [$user_id, $first_name, $last_name, $department_id, $designation_id, $date_of_joining, $status, $employee_id, $company_id];
        }
    ...
    case 'delete':
        $employee_id = isset($_POST['employee_id']) ? (int) $_POST['employee_id'] : 0;
        if ($employee_id > 0) {
            // Extra security: ensure the employee being deleted belongs to the admin's company
