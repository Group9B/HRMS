...
class NexusBot {
		...
		this.quickActions.forEach((btn) => {
			btn.addEventListener("click", async () => {
				...
				if (query) {
					...
					if (
						query.toLowerCase().includes("clock in") ||
						query.toLowerCase().includes("clock out")
					) {
	...
	addMessage(content, sender, type = "text", source = null, widget = null) {
		...
		if (type === "security")
			icon =
				'<i class="ti ti-shield-lock" style="color: #dc3545; margin-right: 8px;"></i>';
		if (type === "error")
			icon =
				'<i class="ti ti-alert-circle" style="color: #dc3545; margin-right: 8px;"></i>';
		if (type === "action")
			icon =
				'<i class="ti ti-bolt" style="color: #ffc107; margin-right: 8px;"></i>';
		if (type === "system")
			icon =
				'<i class="ti ti-settings" style="color: #6c757d; margin-right: 8px;"></i>';
		...
		btns.forEach((btn) => {
			btn.addEventListener("click", () => {
				...
				if (action === "apply_leave")
					window.location.href = "/hrms/employee/leave.php";
			});
		});
	}
	...
	renderWidget(widget) {
		...
		switch (widget.type) {
			case "attendance":
				return this.renderAttendanceWidget(widget);
			case "leave":
				return this.renderLeaveWidget(widget);
			case "tasks":
				return this.renderTasksWidget(widget);
			case "team":
				return this.renderTeamWidget(widget);
			default:
				return "";
		}
	}
	...
	renderAttendanceWidget(data) {
		...
		const checkIn = data.today.check_in
			? data.today.check_in.substring(0, 5)
			: "--:--";
		const checkOut = data.today.check_out
			? data.today.check_out.substring(0, 5)
			: "--:--";
	...
	renderLeaveWidget(data) {
		let balancesHtml = data.balances
			.map(
				(b) => `
            <div class="mb-2">
